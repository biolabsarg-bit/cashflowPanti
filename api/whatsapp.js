...
async function clasificar(texto, cajas, cats, cobrarPend) {
  const hoy = HOY();
...
3. "cobrar_hecho" — marca como COBRADA una cuenta por cobrar pendiente (ya entró la plata).
   {"tipo":"cobrar_hecho","quien":"Logística","caja_id":123|null}
   - Disparadores: "ya cobramos lo de X", "pagó X", "entró lo de X". Matcheá "quien" con la lista de CUENTAS POR COBRAR PENDIENTES.
   - Si dice a qué caja/cuenta entró la plata, poné caja_id; si no, null.
4. "consulta" — {"tipo":"consulta","que":"saldos"|"ingresos_mes"|"egresos_mes"|"saldo_caja"|"por_categoria"|"a_cobrar"|"retiros","caja_id":null,"categoria_id":null}
   ...
   "cuánto nos deben / cuánto hay por cobrar"→a_cobrar; "cuánto retiró cada uno / cuánto saqué"→retiros.
5. "otro" — {"tipo":"otro"}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const c = await clasificar(texto, cajas, cats, cobrarPend);
    ...
    if (c.tipo === "cobrar" && c.monto && c.quien) {
      const fecha = (c.fecha && /^\d{4}-\d{2}-\d{2}$/.test(c.fecha)) ? c.fecha : HOY();
      const obj = { id:Date.now(), quien:c.quien, monto:c.monto, descripcion:c.descripcion||"", fecha, cobrado:false, autor, creado_en:new Date().toISOString() };
      const ok = await sbInsert("cobrar", obj);
      if (!ok) return res.status(200).send(twiml("⚠️ No se pudo guardar la cuenta por cobrar."));
      const txt = `💰 A cobrar registrado: ${c.quien} debe ${fmt(c.monto)}${c.descripcion?` (${c.descripcion})`:""}`;
      if (otroNum) await avisar(otroNum, `📥 ${autor} anotó una cuenta por cobrar:\n${c.quien}: ${fmt(c.monto)}${c.descripcion?` (${c.descripcion})`:""}`);
      return res.status(200).send(twiml(txt));
    }

    // ---- MARCAR COBRADO ----
    if (c.tipo === "cobrar_hecho" && c.quien) {
      const q = String(c.quien).toLowerCase();
      const match = (cobrarPend||[]).find(x => x.quien.toLowerCase().includes(q) || q.includes(x.quien.toLowerCase()));
      if (!match) return res.status(200).send(twiml(`⚠️ No encontré una cuenta pendiente de "${c.quien}". Pendientes: ${ (cobrarPend||[]).map(x=>x.quien).join(", ") || "ninguna" }.`));
      await sbPatch("cobrar", match.id, { cobrado:true });
      // Si indicó caja, registra el ingreso
      let ingTxt = "";
      const caja = c.caja_id ? cajas.find(x=>x.id===Number(c.caja_id)) : null;
      if (caja) {
        await sbInsert("movimientos", {
          id:Date.now(), tipo:"ingreso", monto:Number(match.monto), caja_id:caja.id,
          caja_destino_id:null, categoria_id:null,
          descripcion:`Cobro: ${match.quien}${match.descripcion?` - ${match.descripcion}`:""}`,
          fecha:HOY(), autor, es_retiro:false, creado_en:new Date().toISOString()
        });
        ingTxt = `\n⬇️ Ingreso de ${fmt(match.monto,caja.moneda)} en ${caja.nombre}.`;
      }
      const txt = `✅ Cobrado: ${match.quien} (${fmt(match.monto)}).${ingTxt}${caja?"":"\n(No registré ingreso porque no indicaste la caja.)"}`;
      if (otroNum) await avisar(otroNum, `📥 ${autor} marcó como cobrado:\n${match.quien}: ${fmt(match.monto)}${caja?` → ${caja.nombre}`:""}`);
      return res.status(200).send(twiml(txt));
    }
    ...
    if (c.tipo === "consulta") {
      ...
      if (c.que === "a_cobrar") {
        const total = (cobrarPend||[]).reduce((s,x)=>s+Number(x.monto),0);
        if ((cobrarPend||[]).length===0) return res.status(200).send(twiml("✅ No hay cuentas pendientes de cobro."));
        const lineas = cobrarPend.map(x=>`• ${x.quien}: ${fmt(x.monto)}${x.descripcion?` (${x.descripcion})`:""}`).join("\n");
        return res.status(200).send(twiml(`💰 A cobrar (${fmt(total)}):\n${lineas}`));
      }
      if (c.que === "retiros") {
        ...
        const lineas = socios.map(s=>`• ${s}: ${fmt(ret[s]||0)}`).join("\n");
        let extra = "";
        if (socios.length===2) {
          const a=ret[socios[0]]||0, b=ret[socios[1]]||0, dif=Math.abs(a-b);
          if (dif>0) extra = `\n${a<b?socios[0]:socios[1]} tiene ${fmt(dif)} a favor para igualar.`;
        }
        return res.status(200).send(twiml(`↩️ Retiros de socios:\n${lineas}${extra}`));
      }
      return res.status(200).send(twiml("No entendí la consulta. Probá: '¿cuánto hay en caja?', '¿cuánto nos deben?' o '¿cuánto retiró cada uno?'"));
    }

    return res.status(200).send(twiml("Mandame un movimiento ('pagué 50000 a proveedor en efectivo'), un retiro ('retiré 100000 del banco'), una cuenta por cobrar ('logística nos debe 200000') o una consulta ('¿cuánto nos deben?')."));
